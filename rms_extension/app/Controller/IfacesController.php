 ...
 * @return null
 */
	public function view($id = null, $environmentID = null, $bindTime = null) {
		if (!$id) {
			// no ID provided
			throw new NotFoundException('Invalid interface.');
		}
		...
		if (!$bindTime) {
			// no environment ID provided
			throw new NotFoundException('No bind time given.');
		}
		...
		$this->DispatcherClient->bindToMe($env['Rosbridge']['host'], $bindTime);
